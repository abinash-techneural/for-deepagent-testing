...
func main() {
	...
	queue, err1 := rabbitChannel.QueueDeclare("shipping_queue", true, false, false, false, nil)

	if err1 != nil {
		log.Fatal(err1, "Failed to declare exchange")
	}

	err1 = rabbitChannel.QueueBind("shipping_queue", "", "order_events", false, nil)
	if err1 != nil {
		log.Fatal(err1, "Failed to declare exchange")
	}
	...
	for msg := range chann {
		...
		log.Println("Shipping processing for order:", orderCreatedEvent.OrderID, "items:", orderCreatedEvent.Items)
